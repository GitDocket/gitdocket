...
The homepage keeps the pitch, a compact Homebrew install command as the hero action, current product UI, a short workflow, workflow-extension value and the OKF v0.1 foundation. The Board screenshot expands on demand. The beginner path still starts with one task. Keep setup aligned with `docs/getting-started.md`; verify CLI examples against command help and MCP schemas against a real `tools/list` response.
